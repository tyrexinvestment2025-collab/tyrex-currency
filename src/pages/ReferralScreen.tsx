...
import { Copy, Lock, Share2, RefreshCw, CheckCircle2, ShieldCheck, MinusCircle } from 'lucide-react';
...
const ReferralScreen: React.FC = () => {
    ...
    const handleClaim = async () => {
        ...
        } catch (e) { toast.error("Помилка збору"); } finally { setClaiming(false); }
    };
    ...
    return (
        <div className="min-h-screen bg-[#080808] text-white pb-32 font-sans overflow-x-hidden">
            ...
            <div className="p-5 space-y-5">
                ...
                {/* --- 2. РЕФЕРАЛЬНА ССИЛКА (Тепер ПЕРША) --- */}
                <div className="bg-[#141414] border border-[#222222] p-5 rounded-[2rem] space-y-4 shadow-xl">
                    <div className="flex justify-between items-center px-1">
                        ...
                        <div className="flex gap-2">
                            <button onClick={copyLink} className="p-2.5 bg-[#080808] border border-[#222222] rounded-xl active:border-[#FFB700] transition-all shadow-inner"><Copy size={14} className="text-[#FFB700]"/></button>
                            <button onClick={() => {}} className="p-2.5 bg-[#080808] border border-[#222222] rounded-xl"><Share2 size={14} className="text-[#808080]"/></button>
                        </div>
                    </div>
                ...
                <div className="bg-[#141414] border border-[#FFB700]/20 rounded-[2.5rem] p-7 relative overflow-hidden shadow-2xl">
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center">
                        ...
                        <div className="flex items-baseline gap-2 mb-1">
                            ...
                            <span className="text-sm font-black text-[#FFB700] uppercase">SATS</span>
                        </div>
                    ...
                    {['Результати', 'Партнери'].map((tab, idx) => (
                        <button 
                            ...
                            className={clsx(
                                ...
                                (idx === 0 && activeTab === 'RESULTS') || (idx === 1 && activeTab === 'PARTNERS') ? "bg-white text-black shadow-xl scale-[1.02]" : "text-[#808080]"
                            )}
                        >
                            {tab}
                        </button>
                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'RESULTS' ? (
                        ...
                        <motion.div key="part" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
                             ...
                             {partners.map((p: any) => (
                                <div key={p.id} className="bg-[#141414] border border-[#222222] p-4 rounded-2xl flex items-center justify-between shadow-lg">
                                    <div className="flex items-center space-x-3">
                                        <div className="w-10 h-10 rounded-xl bg-[#080808] border border-[#222222] flex items-center justify-center shadow-inner">
