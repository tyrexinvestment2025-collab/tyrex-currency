        ...
        const load = async () => {
            try {
                ...
                if (res.analytics) {
                    ...
                    // Автоматический выбор первого актива при загрузке или смене категории
                    ...
                    {['traditional', 'crypto'].map((cat) => (
                        <button 
                            ...
                            className={clsx(
                                ...
                                activeCategory === cat ? "bg-[#1C1C1C] text-white shadow-lg" : "text-white/30"
                            )}
                        >
            ...
            <div className="mx-4 mb-6 relative">
                <div className="bg-[#151517] rounded-[2.5rem] p-8 border border-white/5 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <button onClick={() => setShowInfo(true)} className="p-1 active:scale-90 transition-transform">
                    ...
                    {showInfo && (
                        <div className="absolute inset-0 z-50 bg-[#0A0A0B]/95 backdrop-blur-md p-8 flex flex-col justify-center animate-in fade-in duration-200">
                            ...
                            <div className="space-y-4 text-[11px] font-bold text-white/60 uppercase leading-relaxed tracking-tight">
                                ...
                                <p>ЧЕМ БОЛЬШЕ ОБЛАСТЬ ПОКРЫТИЯ — ТЕМ ВЫШЕ ЭФФЕКТИВНОСТЬ СТРАТЕГИИ.</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="h-[280px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            {/* key={...} заставляет график перерисовываться с нуля при каждом изменении выбора */}
                            <RadarChart 
                            ...
                            >
                                ...
                                {/* Анимация для Tyrex — теперь перерисовывается всегда */}
                                <Radar 
                                    dataKey="Tyrex" 
                                    stroke="#FDB931" 
                                    strokeWidth={3} 
                    ...
                    <div className="flex justify-center space-x-8 mt-4">
                        <div className="flex items-center space-x-2"><div className="w-2.5 h-2.5 rounded-full bg-[#FDB931]" /><span className="text-[10px] font-black uppercase text-white/80">Tyrex Score</span></div>
                        <div className="flex items-center space-x-2"><div className="w-2.5 h-2.5 rounded-full bg-[#333]" /><span className="text-[10px] font-black uppercase text-white/30">{selectedAsset}</span></div>
                    </div>
                </div>
            </div>
                ...
                {currentBenchmarks.map((asset: any) => {
                    ...
                    return (
                        <button
                            ...
                            className={clsx(
                                "flex flex-col items-center justify-center py-6 px-2 rounded-[1.8rem] transition-all border-2 active:scale-95",
                                isActive 
